...
public class AbstractInterpretation {
    ...
    private static IJavaObject createNewObject(@NotNull ConstructExpression ce) {
        ...
        switch (name) {
            ...
            default -> newObject = new JavaObject(ce.getType().getTypeName());
        }
        return newObject;
    }
    ...
    public void runMain(@NotNull TranslationUnitDeclaration tud) {
        ...
        JavaObject mainClassVar = new JavaObject(mainClas.toType().getName().toString());
    ...
    private @NotNull Node walkAssignExpression(@NotNull AssignExpression ae) {
        assert !valueStack.isEmpty();
    ...
    private @NotNull Node walkShortCircuitOperator(@NotNull ShortCircuitOperator scop) {
    ...
    private @NotNull Node walkUnaryOperator(@NotNull UnaryOperator uop) {
        ...
        if (uop.getNextEOG().isEmpty()) {      // when throw is the last statement in a block
            ...
            ae.setLhs(Collections.singletonList(new Expression() {
            }));
            ae.setOperatorCode("=");
            ae.setNextEOG(uop.getNextEOG());
            return ae;
        }
    }
    ...
    private void walkNewExpression(@NotNull NewExpression ne) {
        ...
        Declaration classNode = ce.getInstantiates();
        List<IValue> arguments = new ArrayList<>();
    ...
    private Node walkWhileStatement(@NotNull WhileStatement ws) {
        ...
        // continue with the next node after a while
