...
class Graph {
    ...
    private void DFSVisit(No x) {
        ...
        x.visitado = true;
        for (No cursor : x.adj) {
            if (!cursor.visitado) DFSVisit(cursor);
        }
        tempo++;
        x.tempof = tempo;
    ...
    void DFS_T() {
        while (!allVisited()) {
            No TMax = findMax(); //System.out.println(TMax.no);
            int nelementos = NelementosGrupo(TMax); //System.out.println(nelementos);
    ...
    private int NelementosGrupo(No x) {
        ...
        x.visitado = true;
        for (No cursor : x.adj) {
            if (cursor.visitado == false) count += NelementosGrupo(cursor);
        }
        return count;
    }
